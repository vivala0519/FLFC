...
const VoteDetail = (props) => {
  ...
  // style
  const templateStyle = 'absolute w-10/12 h-[92%] bg-white border-2 border-gray-300 rounded-md flex flex-col items-center justify-center z-10'
  const tapDivStyle = 'w-full h-10 flex items-center justify-evenly absolute top-16'
  ...
  return (
      <div className={templateStyle}>
        ...
        <div className={tapDivStyle}>
          <span className={tapSpanStyle + `${tap === 0 ? activeTapStyle : inactiveTapStyle}`} onClick={() => setTap(0)}>참여</span>
          <span className={tapSpanStyle + `${tap === 1 ? activeTapStyle : inactiveTapStyle}`} onClick={() => setTap(1)}>미참여</span>
        </div>
        {tap === 0 && <span className={infoMessageStyle}>투표 시간순으로 표시됩니다</span>}
