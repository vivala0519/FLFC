...
import LetsRecord from '@/components/templates/LetsRecord.jsx'
import StatusBoard from '@/components/templates/StatusBoard.jsx'
import RecordRoom from '@/components/templates/RecordRoom.jsx'
import WeeklyTeam from '@/components/templates/WeeklyTeam.jsx'
...
const MainPage = () => {
  ...
  // const { yearData, updateWeeklyTeamData, updateYearData, updateRtDB } = useStore((state) => state)
  ...
  const dataGeneration = async () => {
    ...
    // yearData['2024'] = fetchedData
    // updateYearData(yearData)
    // updateWeeklyTeamData(fetchedWeeklyTeamData)
  ...
  return (
    <div className='flex flex-col items-center' style={{height: '100vh'}}>
      ...
      <header ref={headerRef} className='flex flex-col items-center w-full top-5'>
        ...
        <div className='flex flex-row justify-around w-full border-double border-0 border-b-2 border-t-2 border-green-500 mb-5 p-2' style={{fontFamily: 'DNFForgedBlade'}}>
          <div
