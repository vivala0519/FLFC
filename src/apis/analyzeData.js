...
export const dataAnalysis = async (quarter, yearParameter) => {
  ...
  activeQuarterStats.forEach((value, key) => {
    ...
    value['일평균득점'] = parseFloat(
      Math.ceil((value['골'] / value['출석']) * 100) / 100,
    )
    value['일평균어시'] = parseFloat(
  ...
  // 공격포인트 순위
  const attackPointRank = totalStatsArray.sort(
    (a, b) => b[1]['공격포인트'] - a[1]['공격포인트'],
  )
  let prevAttackPoint = null,
    prevAttackPointRank = 1
  ...
  // 포인트 총합 순위
  const totalPointRank = totalStatsArray.sort(
    (a, b) => b[1]['포인트총합'] - a[1]['포인트총합'],
  )
  let prevTotalPoint = null,
    prevTotalPointRank = 1
  ...
  const activeQuarterData = {
    ...
    lastSeasonKings: lastKings,
  }

  return {
    active: activeQuarterData,
    totalQuarterData: [
      firstQuarterData,
      secondQuarterData,
      thirdQuarterData,
